...
public class ListenerManager extends HashMap<Class, Listener[]> {
	...
	/**
	 * 接受对象, 执行内容
	 * 该方式会遍历所有接口的继承关系，但是在我们后期开发的时候，
	 * 认为@Include功能能够满足这里的需求，并且提供更快的处理过程，
	 * 所以这里不再需要该方法
	 * @param bean
	 * @return
	 */
	...
	/**
	 * 增加监听对象 不推荐使用该方法，该方法对监听的有效性和监听内容可行性都没有判断 推荐使用addListeners(Listener
	 * listener, Set<Class> classes)作为代替
	 * 
	 * @param listener
	 *            监听对象
	 * @param clazz
	 ...
	 * @return
	 */
	public boolean addListener(Class<Listener> listenerClass) {
		try {
