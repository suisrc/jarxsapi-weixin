...
public class OpenApiServerActivator extends AbstractWeixinActivator implements ApiActivator, WxConfig {
	...
	public void init() {
		...
		baseUrl = System.getProperty(OpenWxConsts.BASE_URL_API, "https://api.weixin.qq.com");
		// 构建缓存线程池
		executor = Executors.newFixedThreadPool(Integer.valueOf(System.getProperty(OpenWxConsts.KEY_ACTIVATOR_THREAD_COUNT_API, "10")));
	...
	public <T> T getAdapter(String key) {
		if( OpenWxConsts.OPEN_ID.equals(key) ) {
			return (T) getOpenId();
		} else if( OpenWxConsts.BASE_URL_API.equals(key) ) {
			return (T) getBaseUrl();
		}
